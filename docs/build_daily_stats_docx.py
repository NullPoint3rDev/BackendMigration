...
add_body(doc, [
    T("Расход проволоки считается только по времени сварки, через скорость подачи и "
      "линейную плотность проволоки."),
])

add_body(doc, [
    B("Данные. "),
    T("Те же состояния за сутки "), M("[dayStart, effectiveEnd)"),
    T(". Для каждого подгружается параметр «Расход проволоки» ("), M("WIRE_PARAM"),
    T(") — это скорость подачи в метрах в минуту (m/min). Если через обычный маппинг "
      "значение не нашлось, есть fallback на нативный SQL-запрос с COALESCE."),
])

add_body(doc, [
    B("Константа. "),
    T("Линейная плотность проволоки "), M("wireLinearDensityKgPerMeter"),
    T(" = 0.000089 кг/м (из конфигурации)."),
])

add_body(doc, [B("Проход по состояниям. "), T("Для каждого состояния:")])
numbered(doc, [T("Учитываются только состояния со статусом Welding (остальные пропускаются).")])
numbered(doc, [T("Считается перекрытие длительности с окном суток (та же "),
               M("overlapDurationMs"), T(", "), M("openEndIfLast = null"),
               T("). Если ≤ 0 — пропуск.")])
numbered(doc, [T("Берётся скорость подачи "), M("mpm"),
               T(" для этого состояния; если её нет или ≤ 0 — пропуск.")])
numbered(doc, [T("Длительность перекрытия переводится в минуты: "),
               M("minutes = overlapMs / 60000"), T(".")])
numbered(doc, [T("Масса на этом отрезке = "), M("mpm × density × minutes"),
               T(", прибавляется к общей сумме.")])

add_body(doc, [T("Формула по сути:")])
code_block(doc, ["масса (кг) = скорость_подачи (м/мин) × плотность (кг/м) × время_сварки (мин)"])

add_body(doc, [
    B("Итог. "),
    T("Сумма по всем сварочным отрезкам округляется до 5 знаков и сохраняется как "),
    M("wireConsumptionKg"), T("."),
])
