...
@Service
public class TcpCore2DeviceClient {
    ...
    // @PostConstruct - отключен, Core2 устройства теперь работают через archive-style TCP сервер
    public void start() {
        System.out.println("[TCP-CORE2] 🚀 Запуск TCP клиента для Core2");
        System.out.println("[TCP-CORE2] Хост: " + host + ":" + port);
        System.out.println("[TCP-CORE2] Ожидаемый MAC: " + coreMac);
        System.out.println("[TCP-CORE2] Таймаут: " + timeoutMs + "мс");

        clientThread = new Thread(() -> {
            while (running) {
                Socket socket = null;
                try {
                    System.out.println("[TCP-CORE2] 🔌 Попытка подключения к " + host + ":" + port + " (попытка " + (retryCount + 1) + ")");

                    socket = new Socket();
                    socket.connect(new java.net.InetSocketAddress(host, port), timeoutMs);

                    System.out.println("[TCP-CORE2] ✅ Подключение установлено к " + host + ":" + port);
                    isConnected = true;
                    ...
                    try (InputStream in = socket.getInputStream()) {
                        ...
                        while (running && (bytesRead = in.read(buffer)) != -1) {
                            ...
                            System.out.println("[TCP-CORE2] 📨 Получены сырые данные (" + bytesRead + " байт): " + data);

                            String mac = extractMacFromPacket(data);
                            if (mac != null) {
                                System.out.println("[TCP-CORE2] 🔍 Извлечен MAC: " + mac);
                                if (coreMac.equalsIgnoreCase(mac)) {
                                    if (!data.startsWith("PING:")) {
                                        String msg = "[TCP-CORE2] ✅ Данные от Core2 (" + mac + "): " + data;
                                        System.out.println(msg);
                                        log.info(msg);
                                        processWeldingData(data, mac);
                                    } else {
                                        System.out.println("[TCP-CORE2] ⏭️ Пропущен ping от Core2");
                                    }
                                } else {
                                    String warn = "[TCP-CORE2] ⚠️ Неизвестный MAC: " + mac + " (ожидался: " + coreMac + ")";
                                    System.out.println(warn);
                                    log.warn(warn);
                                }
                            } else {
                                String err = "[TCP-CORE2] ❌ Не удалось извлечь MAC из данных: " + data;
                                System.out.println(err);
                                log.warn(err);
                            }
                        }

                        System.out.println("[TCP-CORE2] 🔌 Соединение закрыто устройством");
                    ...
                    System.err.println("[TCP-CORE2] ❌ Ошибка: " + e.getMessage());
                    log.error("[TCP-CORE2] Ошибка", e);
                    deviceManager.markDeviceDisconnected(coreMac);
                    try {
                        Thread.sleep(retryIntervalMs);
                    } catch (InterruptedException ignored) {}
            ...
            System.out.println("[TCP-CORE2] 🔚 TCP клиент остановлен");
        });
                ...
                try {
                    ...
                    if (isConnected && lastDataReceived > 0) {
                        ...
                        if (timeSinceLastData > 30000) {
                            System.out.println("[TCP-CORE2] ⚠️ Данных не было " + (timeSinceLastData / 1000) + " секунд, отмечаем как отключенный");
                            deviceManager.markDeviceDisconnected(coreMac);
                            isConnected = false;
                        }
                    }
    ...
    private void processWeldingData(String data, String mac) {
        try {
            ...
            System.out.println("[TCP-CORE2] ✅ Данные обработаны и сохранены");
            log.info("[TCP-CORE2] Данные обработаны и сохранены для {}", mac);
        } catch (Exception e) {
            System.err.println("[TCP-CORE2] Ошибка обработки данных: " + e.getMessage());
            log.error("[TCP-CORE2] Ошибка обработки данных для " + mac, e);
        }
    }
    ...
    public void stop() {
        ...
        System.out.println("[TCP-CORE2] Stopped.");
    }
}
